<template>
  <aside
    class="overview-sidebar hidden lg:flex w-[220px] xl:w-[240px] shrink-0 flex-col border-l border-[var(--color-border)] bg-[var(--color-bg-card)]/80 backdrop-blur-[var(--blur-glass)]"
    style="min-height: 100vh"
  >
    <div class="overview-inner flex flex-col flex-1 p-5">
      <div
        class="overview-header mb-4 flex items-center gap-2 border-b border-[var(--color-border)] pb-3"
      >
        <span
        ...
        <h3
          class="text-[13px] font-semibold tracking-tight text-[var(--color-text)]"
        >
          Overview
        </h3>
      </div>
      ...
      <div v-if="route.name === 'home'" class="overview-cards space-y-3">
        ...
        <div
          ...
          :class="{ 'overview-card-wrap--has-rejected': listSummary.rejectedCount > 0 }"
        >
          <span
