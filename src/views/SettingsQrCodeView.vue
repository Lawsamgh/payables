    ...
    <section class="qr-url-section w-full">
      ...
      <div v-if="!initialLoading" class="flex gap-3">
        ...
        <button
          ...
          :disabled="
            saving || urlInput === (documentSettings.vendorCollectQrUrl ?? '')
          "
          @click="onSave"
        >
          {{ saving ? "Saving…" : "Save" }}
...
async function onSave() {
  ...
  saving.value = true;
  try {
    const { error } = await documentSettings.saveVendorCollectQrUrl(
      urlInput.value,
    );
    if (error) {
      toast.error(error);
      ...
      toast.success(
        "URL saved. The QR code on the Cheque collection page will use this URL.",
      );
    }
  } finally {
    saving.value = false;
...
async function onDownloadPdf() {
  ...
  try {
    ...
    const pdfMake = ((pdfMakeModule as { default?: PdfMakeApi }).default ??
    ...
    const docDefinition = {
      ...
      content: [
        ...
        {
          text: "Scan the QR code with your mobile device to record cheque collection",
          style: "subtitle",
        },
        {
          text: "This page is designed for mobile use. Use your phone's camera or a QR scanner app.",
          fontSize: 10,
          color: "#64748b",
          margin: [0, 0, 0, 16],
          italics: true,
        ...
        {
          ol: [
            "Scan the QR code above with your mobile phone camera or QR scanner app. (Desktop browsers will see a 'Mobile only' message.)",
            "Enter your TransRef (sent to you in your email, e.g. PAY199030739500) and tap Continue.",
            "Enter your Code manually or scan the QR code the officer shows you. Tap Continue.",
