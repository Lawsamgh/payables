<template>
  ...
  >
    ...
    <div v-if="loading" class="flex flex-col gap-10">
      ...
      <section class="flex flex-col gap-3">
        ...
        <ul class="list-none p-0 m-0 flex flex-col gap-2">
          ...
          >
            <Skeleton width="1.25rem" height="1.25rem" circle class="shrink-0" />
            ...
            <Skeleton width="4.5rem" height="0.75rem" />
            <Skeleton width="3rem" height="1.75rem" class="rounded-full ml-auto" />
          </li>
        </ul>
      </section>
    </div>
    ...
    <div v-else class="flex flex-col gap-10">
      <!-- Greeting + Stats dashboard: KPIs + donut chart (no distribution bar) -->
      <div class="stats-dashboard">
        ...
        <div class="stats-dashboard__kpis">
          ...
          <div class="stat-card-wrap stat-card-wrap--rejected" :class="{ 'stat-card-wrap--has-rejected': displayedRejected > 0 }">
            <span
            ...
            <button
              ...
              :aria-label="displayedRejected > 1 ? `Open ${displayedRejected} rejected entries in booklet` : displayedRejected === 1 ? 'Open rejected entry' : 'No rejected entries'"
        ...
        <div class="stats-dashboard__charts">
          <div class="stats-dashboard__chart-wrap">
            <PayablesChart :draft-count="draftList.length" :posted-count="postedList.length" :size="120" />
          </div>
        </div>
      </div>
      ...
      <section class="daily-chart-section daily-chart-section--with-calendar">
        <div class="chart-calendar">
          ...
          <div class="chart-calendar__weekdays">
            <span v-for="w in ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']" :key="w" class="chart-calendar__weekday">{{ w }}</span>
          </div>
            ...
            <template v-for="(week, wi) in calendarGrid" :key="wi">
              <button
                ...
                :class="{
                  ...
                  'chart-calendar__day--today': cell?.isToday && !cell?.isSelected,
                }"
                :disabled="!cell"
          ...
          <button type="button" class="chart-calendar__today" @click="goToToday">Today</button>
        </div>
        <div class="chart-calendar__chart">
          <div class="daily-chart-section__toggle">
            <button
              ...
              :class="{ 'daily-chart-metric-btn--active': dailyChartMode === 'posted' }"
            ...
            <button
              type="button"
              class="daily-chart-metric-btn"
              :class="{ 'daily-chart-metric-btn--active': dailyChartMode === 'approved' }"
          ...
          <DailyPostedChart
            :data="dailyChartMode === 'posted' ? postedByVendor : approvedByVendor"
            :currency="dailyChartMode === 'posted' ? postedCurrency : approvedCurrency"
            :total-label="dailyChartMode === 'posted' ? postedTodayTotalsLabel : approvedTodayTotalsLabel"
      ...
      <div class="search-and-tabs">
        <div class="search-bar search-bar--compact">
          <div class="search-bar__wrap" :class="{ 'search-bar__wrap--has-value': searchQuery }">
          ...
          <p v-if="searchQuery" class="search-bar__hint">
            <template v-if="hasResultsInAnyTab">
              <span v-if="filteredDraftList.length > 0">{{ filteredDraftList.length }} in Draft</span>
              <template v-if="filteredDraftList.length > 0 && (filteredPostedList.length > 0 || filteredRejectedList.length > 0 || filteredApprovedList.length > 0)"> · </template>
              <span v-if="filteredPostedList.length > 0">{{ filteredPostedList.length }} in Posted</span>
              <template v-if="filteredPostedList.length > 0 && (filteredRejectedList.length > 0 || filteredApprovedList.length > 0)"> · </template>
              <span v-if="filteredRejectedList.length > 0">{{ filteredRejectedList.length }} in Rejected</span>
              <template v-if="filteredRejectedList.length > 0 && filteredApprovedList.length > 0"> · </template>
              <span v-if="filteredApprovedList.length > 0">{{ filteredApprovedList.length }} in Approved</span>
              <span v-if="currentTabHasNoResults && hasResultsInAnyTab" class="search-bar__hint-switch">
        ...
        <div class="segment-control" role="tablist" aria-label="List by status">
        <button
          type="button"
          role="tab"
          ...
          :class="{ 'segment-control__segment--active': activeSegment === 'draft' }"
          @click="activeSegment = 'draft'"
        >
          <span class="segment-control__label">Draft</span>
          <span class="segment-control__badge">{{ searchQuery ? `${filteredDraftList.length} of ${draftList.length}` : draftList.length }}</span>
        </button>
        <button
          type="button"
          role="tab"
          ...
          id="tab-posted"
          class="segment-control__segment"
          :class="{ 'segment-control__segment--active': activeSegment === 'posted' }"
          @click="activeSegment = 'posted'"
        >
          <span class="segment-control__label">Posted</span>
          <span class="segment-control__badge">{{ searchQuery ? `${filteredPostedList.length} of ${postedList.length}` : postedList.length }}</span>
        </button>
        <button
          type="button"
          role="tab"
          ...
          id="tab-rejected"
          class="segment-control__segment"
          :class="{ 'segment-control__segment--active': activeSegment === 'rejected' }"
          @click="activeSegment = 'rejected'"
        >
          <span class="segment-control__label">Rejected</span>
          <span class="segment-control__badge">{{ searchQuery ? `${filteredRejectedList.length} of ${rejectedList.length}` : rejectedList.length }}</span>
        </button>
        <button
          type="button"
          role="tab"
          ...
          id="tab-approved"
          class="segment-control__segment"
          :class="{ 'segment-control__segment--active': activeSegment === 'approved' }"
          @click="activeSegment = 'approved'"
        >
          <span class="segment-control__label">Approved</span>
          <span class="segment-control__badge">{{ searchQuery ? `${filteredApprovedList.length} of ${approvedList.length}` : approvedList.length }}</span>
        </button>
        </div>
      </div>
      ...
      >
          ...
          >
            ...
            <div class="list-row__meta">
              <div class="list-row__meta-col">
                ...
                <span class="list-row__meta-value truncate" :title="item.fieldData.VendorID || ''">{{ item.fieldData.VendorID || "—" }}</span>
              </div>
              <div class="list-row__meta-col">
                <span class="list-row__meta-label">Ref</span>
                <span class="list-row__meta-value truncate" :title="item.fieldData.TransRef || ''">{{ item.fieldData.TransRef || "—" }}</span>
              </div>
              <div class="list-row__meta-col list-row__meta-col--total">
                <span class="list-row__meta-label">Total</span>
                <span class="list-row__meta-value list-row__meta-value--total tabular-nums">{{ item.fieldData.Total != null ? `${item.fieldData.Currency ?? ''} ${formatNumberDisplay(item.fieldData.Total)}`.trim() : '—' }}</span>
              </div>
            </div>
            <router-link
              :to="{
        ...
        <div v-if="totalPagesDraft > 1" class="pagination">
          ...
          <button
            ...
            @click="currentPageDraft = Math.min(totalPagesDraft, currentPageDraft + 1)"
          >
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        ...
        <div v-if="totalPagesPosted > 1" class="pagination">
          ...
          <button
            ...
            aria-label="Next page"
            @click="currentPagePosted = Math.min(totalPagesPosted, currentPagePosted + 1)"
          >
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <p
          ...
          class="rounded-2xl border border-dashed border-[var(--color-border)] py-8 px-4 text-center text-[var(--label-size)] text-[var(--color-text-muted)]"
        >
          {{ searchQuery ? 'No matching posted entries.' : 'No posted payables yet.' }}
        </p>
      </section>
      ...
      >
        <div v-if="filteredRejectedList.length > 0" class="list-view--inset">
          <div
            ...
            class="list-view__row list-row group px-4 py-3.5"
          >
            <span class="font-medium text-[var(--color-text)] min-w-0 truncate">{{ item.fieldData.VendorName || "—" }}</span>
            <div class="list-row__meta">
              <div class="list-row__meta-col">
                <span class="list-row__meta-label">Vendor ID</span>
                <span class="list-row__meta-value truncate" :title="item.fieldData.VendorID || ''">{{ item.fieldData.VendorID || "—" }}</span>
            ...
            <router-link
              :to="{ name: 'entry', query: { transRef: item.fieldData.TransRef ?? '' } }"
              class="pill-btn glass-input inline-flex items-center justify-end gap-1.5 px-3.5 py-2 text-[var(--label-size)] font-medium text-[var(--color-accent)] no-underline opacity-90 group-hover:opacity-100 transition-opacity justify-self-end"
            >
              Open
        ...
        <div v-if="totalPagesRejected > 1" class="pagination">
          <button type="button" class="pagination__btn" :disabled="currentPageRejected <= 1" aria-label="Previous page" @click="currentPageRejected = Math.max(1, currentPageRejected - 1)">
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
          </button>
          <span class="pagination__label">Page {{ currentPageRejected }} of {{ totalPagesRejected }}</span>
          <button type="button" class="pagination__btn" :disabled="currentPageRejected >= totalPagesRejected" aria-label="Next page" @click="currentPageRejected = Math.min(totalPagesRejected, currentPageRejected + 1)">
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
          </button>
        </div>
        <p v-if="filteredRejectedList.length === 0" class="rounded-2xl border border-dashed border-[var(--color-border)] py-8 px-4 text-center text-[var(--label-size)] text-[var(--color-text-muted)]">
          {{ searchQuery ? 'No matching rejected entries.' : 'No rejected payables.' }}
        </p>
      </section>
      ...
      >
        ...
        <div v-if="totalPagesApproved > 1" class="pagination">
          <button type="button" class="pagination__btn" :disabled="currentPageApproved <= 1" aria-label="Previous page" @click="currentPageApproved = Math.max(1, currentPageApproved - 1)">
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
          </button>
          <span class="pagination__label">Page {{ currentPageApproved }} of {{ totalPagesApproved }}</span>
          <button type="button" class="pagination__btn" :disabled="currentPageApproved >= totalPagesApproved" aria-label="Next page" @click="currentPageApproved = Math.min(totalPagesApproved, currentPageApproved + 1)">
            <svg class="pagination__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
          </button>
        </div>
        <p v-if="filteredApprovedList.length === 0" class="rounded-2xl border border-dashed border-[var(--color-border)] py-8 px-4 text-center text-[var(--label-size)] text-[var(--color-text-muted)]">
          {{ searchQuery ? 'No matching approved entries.' : 'No approved payables.' }}
        </p>
      </section>
      </div>
    </div>
...
import PayablesChart from "../components/PayablesChart.vue";
...
const activeSegment = ref<"draft" | "posted" | "rejected" | "approved">("draft");
...
function getCreationTimestamp(item: FindRecordWithId<PayablesMainFieldData>): string {
...
function getApprovedDate(item: FindRecordWithId<PayablesMainFieldData>): string {
  const fd = item?.fieldData as Record<string, unknown> | undefined;
  if (!fd) return "";
  const raw =
    fd.ApprovedDate ??
...
const rejectedList = computed(() => filterByStatusAndSort(list.value, "Rejected"));
const approvedList = computed(() => filterByStatusAndSort(list.value, "Approved"));
...
const dailyPostedByCurrency = computed(() => {
  ...
  byCurrency.forEach((dateMap, currency) => {
    result[currency] = Array.from(dateMap.entries()).map(([date, total]) => ({ date, total }));
  });
  return result;
});
...
const calendarGrid = computed(() => {
  ...
  const cells: { dateKey: string; day: number; isSelected: boolean; isToday: boolean } | null[] = [];
...
const postedTodayTotalsLabel = computed(() => {
  ...
  const parts = Object.entries(byCur)
    .map(([currency, sum]) => `${currency} ${formatNumberDisplay(sum) || "0.00"}`)
    .filter(Boolean);
  return parts.length > 0 ? parts.join(" · ") : "";
});
...
watch(
  ...
  ([draft, posted, rejected, approved]) => {
...
watch(totalPagesDraft, (total) => {
  if (currentPageDraft.value > total) currentPageDraft.value = Math.max(1, total);
});
watch(totalPagesPosted, (total) => {
  if (currentPagePosted.value > total) currentPagePosted.value = Math.max(1, total);
});
watch(totalPagesRejected, (total) => {
  if (currentPageRejected.value > total) currentPageRejected.value = Math.max(1, total);
});
watch(totalPagesApproved, (total) => {
  if (currentPageApproved.value > total) currentPageApproved.value = Math.max(1, total);
});
...
watch(
  [draftTotalsByCurrency, postedTotalsByCurrency, rejectedTotalsByCurrency, approvedTotalsByCurrency],
  () => {
...
async function load() {
  ...
  loading.value = true;
  error.value = null;
  const { data, error: err } = await findRecordsWithIds<PayablesMainFieldData>(
