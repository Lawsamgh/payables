  ...
  <div class="content-area flex flex-col flex-1 min-h-0 w-full max-w-[1600px] mx-auto px-4 py-5 md:px-6 md:py-6">
    <header class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div class="flex flex-wrap items-center gap-3">
        ...
        <h1 class="text-xl font-bold tracking-tight text-[var(--color-text)]">Vendors</h1>
      </div>
      <button
        type="button"
    ...
    <!-- Search -->
    <div v-if="!loading && !loadError && vendorList.length > 0" class="tax-search-bar">
      ...
      <p v-if="searchQuery" class="tax-search-bar__hint">
        {{ filteredVendorList.length }} result{{ filteredVendorList.length === 1 ? '' : 's' }}
      </p>
    </div>
    ...
    <div class="tax-table-wrap">
      ...
      <div v-else class="tax-table-scroll">
        <table class="tax-table">
          ...
          <tbody>
            <tr v-for="(row, index) in filteredVendorList" :key="row.recordId || index">
    ...
    <Teleport to="body">
      <div v-if="showAddModal" class="tax-modal-backdrop">
        <div class="tax-modal vendor-modal" role="dialog" :aria-labelledby="editingRecordId ? 'edit-vendor-title' : 'add-vendor-title'" aria-modal="true">
          <header class="tax-modal__header">
            <h2 :id="editingRecordId ? 'edit-vendor-title' : 'add-vendor-title'" class="tax-modal__title">{{ editingRecordId ? 'Edit Vendor' : 'Add Vendor' }}</h2>
            <button
              type="button"
          ...
          <form class="tax-modal__form" @submit.prevent="submit">
            <section class="tax-modal__section">
              ...
              <label class="tax-modal__label">
                <span>Vendor ID</span>
                <input v-model="form.Vendor_ID" type="text" class="glass-input w-full px-3 py-2.5 rounded-lg" placeholder="Vendor ID" />
              </label>
              <label class="tax-modal__label">
                <span>Vendor Name</span>
                <input v-model="form.Vendor_Name" type="text" class="glass-input w-full px-3 py-2.5 rounded-lg" placeholder="Vendor name" required />
              </label>
              <label class="tax-modal__label">
                <span>Vendor Location</span>
                <input v-model="form.Vendor_Location" type="text" class="glass-input w-full px-3 py-2.5 rounded-lg" placeholder="Location" />
              </label>
              <label class="tax-modal__label">
                <span>Vendor Email</span>
                <input v-model="form.Vendor_Email" type="email" class="glass-input w-full px-3 py-2.5 rounded-lg" placeholder="email@example.com" />
              </label>
              <label class="tax-modal__label">
                <span>Tin Number</span>
                <input v-model="form.Tin_Number" type="text" class="glass-input w-full px-3 py-2.5 rounded-lg" placeholder="TIN" />
              </label>
            </section>
            <section class="tax-modal__section">
              ...
              <div class="tax-modal__row">
                <label class="tax-modal__label">
                  ...
                  <input v-model="form.GRA_Expiry_Date" type="date" class="glass-input w-full px-3 py-2.5 rounded-lg" />
                </label>
                <label class="tax-modal__label">
                  <span>Received Date</span>
                  <input v-model="form.Received_Date" type="date" class="glass-input w-full px-3 py-2.5 rounded-lg" />
                </label>
              </div>
              <div class="tax-modal__row">
                <label class="tax-modal__label">
                  <span>WHT Expiry Date</span>
                  <input v-model="form.WHT_Expiry_Date" type="date" class="glass-input w-full px-3 py-2.5 rounded-lg" />
                </label>
                <label class="tax-modal__label">
                  <span>Received WHT Date</span>
                  <input v-model="form.Received_WHT_Date" type="date" class="glass-input w-full px-3 py-2.5 rounded-lg" />
                </label>
              </div>
            </section>
            <div class="tax-modal__actions">
              <button type="button" class="tax-modal__btn-cancel" @click="closeModal">
                Cancel
              </button>
...
function getField(row: FindRecordWithId<VendorTblFieldData | Record<string, unknown>>, key: string): string {
...
const filteredVendorList = computed(() => {
  ...
  return vendorList.value.filter((r) => {
    ...
    const name = String(fd.Vendor_Name ?? fd['Vendor Name'] ?? '').toLowerCase()
    const loc = String(fd.Vendor_Location ?? fd['Vendor Location'] ?? '').toLowerCase()
    const email = String(fd.Vendor_Email ?? fd['Vendor Email'] ?? '').toLowerCase()
    const tin = String(fd.Tin_Number ?? fd['Tin Number'] ?? '').toLowerCase()
    return id.includes(q) || name.includes(q) || loc.includes(q) || email.includes(q) || tin.includes(q)
  })
})
...
function openEdit(row: FindRecordWithId<VendorTblFieldData | Record<string, unknown>>) {
  const fd = row.fieldData as Record<string, unknown>
  form.value = {
    ...
    GRA_Expiry_Date: fileMakerDateToInput(fd.GRA_Expiry_Date ?? fd['GRA Expiry Date'] as string | undefined),
    Received_Date: fileMakerDateToInput(fd.Received_Date ?? fd['Received Date'] as string | undefined),
    Tin_Number: String(fd.Tin_Number ?? fd['Tin Number'] ?? ''),
    WHT_Expiry_Date: fileMakerDateToInput(fd.WHT_Expiry_Date ?? fd['WHT Expiry Date'] as string | undefined),
    Received_WHT_Date: fileMakerDateToInput(fd.Received_WHT_Date ?? fd['Received WHT Date'] as string | undefined),
  }
...
async function submit() {
  ...
  try {
    ...
    if (idToUpdate) {
      const { error } = await updateRecord(LAYOUTS.VENDOR_TBL, idToUpdate, fieldData)
      if (error) {
...
async function loadVendors() {
  ...
  const { data, error } = await findRecordsWithIds<VendorTblFieldData | Record<string, unknown>>(LAYOUTS.VENDOR_TBL, { limit: 500 })
  loading.value = false
  if (error) {
