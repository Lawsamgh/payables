...
/** Match item by search (purchase order, vendor name, ID, invoice #, ref). */
export function matchesSearch(
  ...
  const invoice = String(
    fd?.InvoiceNumber ?? fd?.Invoice_Number ?? "",
  ).toLowerCase();
  const ref = String(item.fieldData?.TransRef ?? "").toLowerCase();
  return (
    purchaseOrder.includes(s) ||
    name.includes(s) ||
    vendorId.includes(s) ||
    invoice.includes(s) ||
    ref.includes(s)
...
export function getModificationTimestamp(
  ...
  for (const [key, val] of Object.entries(fd)) {
    if (
      val != null &&
      String(val).trim() &&
      /modification|modified|date\s*modified/i.test(key)
    ) {
...
/** True when the record has Posted = "Yes" (the actual field, not Status calculation). */
export function isPostedRecord(
  item: FindRecordWithId<PayablesMainFieldData>,
): boolean {
  const fd = item?.fieldData as Record<string, unknown> | undefined;
  if (!fd) return false;
  let posted: unknown = fd.Posted ?? fd.posted ?? fd["Posted"];
  if (posted == null || posted === "") {
    for (const [key, val] of Object.entries(fd)) {
      if (/^posted$/i.test(key.replace(/\s/g, "")) && val != null && String(val).trim() !== "") {
        posted = val;
        break;
      }
    }
